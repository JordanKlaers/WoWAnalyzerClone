...
class FightSelection extends React.PureComponent {
  ...
  //   const { reports, refreshReport } = this.props;
  //   const { killsOnly } = this.state;

  //   const reportDuration = reports.end - reports.start;
  ...
  // }
  render() {
    const { reports, fightIds } = this.props;

    const fights = reports.map((report, index) => getFightFromReport(report, fightIds[index]));
    // if (!fightId || !fight) {
    ...
    return <>
      ...
      {this.props.children(fights)}
    </>;
  }
}

const mapStateToProps = (state, props) => {
  console.log('fight selection props: ', props);
  return {
    // Because fightId comes from the URL we can't use local state
    fightIds: [...props.reports].map(report => {
      console.log('is it getting the IDs:', getFightId(report.reportURL));
      return getFightId(report.reportURL);
    })
  };
}
