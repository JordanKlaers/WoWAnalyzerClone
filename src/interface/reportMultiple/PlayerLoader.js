...
class PlayerLoader extends React.PureComponent {
  ...
  static getDerivedStateFromProps(props, state) {
    //TODO: understand this reset mechanism
  ...
  componentDidMount() {
    ...
    this.loadCombatants(this.props.reports, this.props.fights);
    this.scrollToTop();
  }

  componentDidUpdate(prevProps, prevState, prevContext) {

    const changedReport = this.props.reports.length !== prevProps.reports.length;
    //wont be changing fights
    // const changedFight = this.props.fight !== prevProps.fight;
    if (changedReport) {
      // noinspection JSIgnoredPromiseFromCall
      this.loadCombatants(this.props.reports, this.props.fights);
    }
    //dont need a player as this is multiple reports
  ...
  async loadCombatants(reports, fights) {
    if (reports.gameVersion === 2) {
      return;
    }
    // try {
  ...
  render() {
    const { reports, fights, playerName, playerId } = this.props;
    ...
    const maxEnd = reports.reduce((acc,cur) => {
      return Math.max(acc, cur.end);
    }, 0);
    const minStart = reports.reduce((acc,cur) => {
      return Math.min(acc, cur.start);
    }, Infinity);
    const reportDuration = maxEnd - minStart;
    ...
    return <>
      ...
      <DocumentTitle title={t({
        ...
        message: `${getFightName(reports, fights)} by ${player.name} in ${reports.title}`
      })} />
